        ...
        for URL in final_urlday:
            ...
            for content_url in post:  # 기사 URL
                ...
                try:
                    ...
                    ###이미지 가져옴
                    img_span = document_content.find("span", {'class':'end_photo_org'})
                    img_tag = img_span.find("img")
                    img_url = ''
                    img_url = img_url + str(img_tag.get('src'))
                    
                    if not img_url:  # 공백일 경우 기사 제외 처리
                        continue
                    
                    # CSV 작성
                    wcsv.writerow([news_date, category_name, text_company, text_headline, text_sentence, content_url, img_url])
...
if __name__ == "__main__":
    ...
    Crawler.set_category("정치","경제","생활문화","IT과학","사회")  # 정치, 경제, 생활문화, IT과학, 사회 카테고리 사용 가능
    Crawler.set_date_range(2019, 2019, 1)  # 2017년 1월부터 2018년 4월까지 크롤링 시작
    Crawler.start()
